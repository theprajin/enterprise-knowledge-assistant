...
DATABASE_URL = os.getenv(
    "DATABASE_URL", "postgresql://postgres:postgres@db:5432/knowledge_assistant"
)
...
def get_vector_engine() -> PGEngine:
    global _engine
    if _engine is None:
        _engine = PGEngine.from_connection_string(CONNECTION_URI)
    return _engine
...
def get_vector_store(collection_name: str = "enterprise_knowledge") -> PGVectorStore:
    ...
    # gemini-embedding-001 produces embeddings of size 3072.
    try:
        engine.init_vectorstore_table(table_name=collection_name, vector_size=3072)
    except Exception:
    ...
    return PGVectorStore.create_sync(
        engine=engine, table_name=collection_name, embedding_service=embeddings
    )
